...
<body>
    ...
    <form id="formAction" method="POST" action="{{ route('add_empl_post') }}">
        @csrf
        <div class="container">
            <div class="d-flex align-items-center justify-content-center">
                <div class="col-6 d-flex m-2 p-3 flex-column"
                    ...
                    <div>
                        <select name="selectMO" class="form-select col-2" aria-label="Default select example" id="MOselect"
                            onchange='changeSelect(this)'>
                            @foreach ($MOname as $mo)
                                <option value={{ $loop->iteration }}>{{ $mo }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col d-flex align-items-center justify-content-center">
                        <input name="surname" required type="text" class="form-control me-1 mb-1" placeholder="Фамилия">
                        <input name="name" required type="text" class="form-control ms-1 me-1 mb-1" placeholder="Имя">
                        <input name="secondname" required type="text" class="form-control ms-1 mb-1" placeholder="Отчество">
                    </div>
                    <div class="mt-2 mb-1">Выберите образовательную организацию:</div>
                    <div>
                        <select id="job" required name="selectOO" class="form-select col-2" aria-label="Default select example">
                            
                        </select>
                    </div>
                    <div class="form-check mt-1 mb-1">
                        <input name="cbOO" id="checkJob" class="form-check-input" type="checkbox" id="checkJob"
                    ...
                    <div>
                        <input name="anotherJobI" id="anotherJobI" type="text" class="form-control mb-1" placeholder="Место работы"
                            style="display: none">
                    </div>
                    <div class="mt-2 mb-1">Выберите должность в ППЭ:</div>
                    <div>
                        <select name="multiSelect[]" required class="form-control select2 col-12" multiple="multiple" name="postSelect"
                            id="postSelect">
                            @foreach ($positions as $pos)
                                <option value={{ $loop->iteration }}>{{ $pos }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="d-flex align-items-center justify-content-center">
                        <div>
                            <button class="btn btn-lg btn-primary mt-4" id="button">Добавить работника</button>
                        </div>
                    </div>
                </div>
            </div>
