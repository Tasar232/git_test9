...
class PpeWorkers extends Migration
{
    ...
    public function up()
    {
        Schema::create('ppe_workers', function (Blueprint $table){
            ...
            $table->unsignedSmallInteger('id_education_org')->nullable(true);

            $table->char('other_place_of_work', 255)->nullable(true);
            $table->char('position', 150)->nullable(false);

            $table->foreign('id_login')->references('id')->on('login_users');
